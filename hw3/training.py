    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            save_checkpoint = True #### changes false to true
...
class RNNTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        #Forward pass
        if self.first_batch:
            y_pred, self.hidden_state = self.model(x, None)
        else:
            y_pred, self.hidden_state = self.model(x, self.hidden_state.detach())
        
        output_flat = torch.flatten(y_pred, start_dim=0, end_dim=1)
